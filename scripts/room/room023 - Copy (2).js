...
room23.nextTurn = function () {
    ...
    if (g.internal.evaphase !== g.internal.lolaphase) {
        if (g.internal.nextTurn === "lola") {
            g.internal.nextTurn = g.internal.speaker = "eva";
            g.internal.whoTurn = "lola";
            drawLola("talk");
            drawEva("sit");
        }
        else {
            g.internal.nextTurn = g.internal.speaker = "lola";
...
room23.tord = function () {
    ...
    if (doTruth)
        chat((g.internal.this.nextTurn === "lola" ? 10 : 12), 23);
    else
        chat((g.internal.this.nextTurn === "lola" ? 8 : 6), 23);
};
...
room23.chatcatch = function (callback) {
    ...
    for (chatCatchLoop = 0; chatCatchLoop < cArray.length; chatCatchLoop++) {
        switch (cArray[chatCatchLoop]) {
            ...
            case "lolahappy":
            case "lolasad":
                room23.drawLola(cArray[chatCatchLoop]);
                break;
            case "evatalk":
            case "evasit":
            case "evascared":
            case "evagiggle":
                room23.drawEva(cArray[chatCatchLoop]);
                break;
            case "bothkiss":
                room23.drawLola(cArray[chatCatchLoop]);
                room23.drawEva(cArray[chatCatchLoop]);

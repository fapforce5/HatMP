...
room26.btnclick = function (name) {
    switch (name) {
        ...
        case "chat_landlord":
            ...
            var llevents = sc.getEventall("landlord");
            if (jobapplyconst < 2)
                chat(0, 26);
            else if (!sc.getEvent("landlord", 1) && sc.getMission("landlord", "spermbank").notStarted)
                chat(3, 26);
...
room26.chat = function (chatID) {
    var cArray = [
        ...
        },
        {
            ...
            button: [
                { chatID: -1, text: "Thanks? ", callback: "passtime level1" }
            ]
        },
        {
            chatID: 2,
